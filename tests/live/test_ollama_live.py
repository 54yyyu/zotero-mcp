...
def test_registry_builds_ollama_ef(ollama_available):
    """resolve_provider("ollama") -> spec.ef_factory(...) is the exact
    production path ChromaClient uses to build the realtime EF."""
    spec, extra = resolve_provider("ollama")
    ef = spec.ef_factory({**extra, "model_name": MODEL, "base_url": ollama_available})
