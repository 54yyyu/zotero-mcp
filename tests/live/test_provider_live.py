...
Every EF is built via ``resolve_provider(provider) + spec.ef_factory(...)``
— the exact path production code (``create_chroma_client``) uses — seeded
with the machine's real production ``embedding_config`` (model name, api_key,
etc.), so these tests exercise the actual configuration in use. Inputs are
kept to a handful of short strings; total live-API cost is a small fraction
of a cent.
"""
...
from zotero_mcp.embeddings.registry import resolve_provider  # noqa: E402
...
class _ProviderLiveTests:
    ...
    def _build_ef(self, production_config, **overrides):
        spec, extra = resolve_provider(self.PROVIDER)
        cfg = {**extra, **production_config, **overrides}
        return spec.ef_factory(cfg)
