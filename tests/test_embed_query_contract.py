...
@pytest.mark.parametrize("cls", CUSTOM_EMBEDDING_FUNCTIONS, ids=lambda c: c.__name__)
def test_embed_query_accepts_input_positionally_too(cls):
    """ChromaDB's protocol declares one positional parameter; keep it working."""
    ef = _make(cls, [])
    assert _rows(ef.embed_query(["only"])) == [[4.0, 1.0]]


@pytest.mark.parametrize("cls", CUSTOM_EMBEDDING_FUNCTIONS, ids=lambda c: c.__name__)
def test_embed_query_of_empty_input_is_empty(cls):
    calls = []
    ef = _make(cls, calls)
    assert ef.embed_query(input=[]) == []
    assert calls == []
...
@pytest.mark.parametrize("cls", CUSTOM_EMBEDDING_FUNCTIONS, ids=lambda c: c.__name__)
def test_embed_query_text_is_the_single_string_helper(cls):
    """The renamed helper still exists and still returns one flat vector."""
    calls = []
    ef = _make(cls, calls)

    vector = ef.embed_query_text("abcd")

    assert list(vector) == [4.0, 1.0]
    assert "text" in inspect.signature(cls.embed_query_text).parameters


# ---------------------------------------------------------------------------
# 4. Gemini: the provider whose query path really differs from its document
#    path must keep doing so through the new batch entry point.
# ---------------------------------------------------------------------------


def _gemini(model_name, requests):
    """A Gemini EF whose genai client records each ``embed_content`` call."""
    ...
    class _Models:
        def embed_content(self, model, contents, **kwargs):
            requests.append({"contents": list(contents), **kwargs})
...
def test_gemini_v1_embed_query_still_requests_the_retrieval_query_task():
    """v1 asymmetry lives in the ``task_type`` config, i.e. in ``is_query``."""
    requests = []
    ef = _gemini("gemini-embedding-001", requests)

    result = ef.embed_query(input=["a query"])

    assert result == [[0.5, 0.5]]
    assert requests == [{"contents": ["a query"], "config": {"task_type": "retrieval_query"}}]


def test_gemini_v2_embed_query_still_prepends_the_query_prefix():
    """v2 asymmetry lives in the prompt text, i.e. in ``_prepare_query``."""
    requests = []
    ef = _gemini("models/gemini-embedding-2-preview", requests)

    ef.embed_query(input=["first", "second"])

    prefix = GeminiEmbeddingFunction.V2_QUERY_PREFIX
    assert [r["contents"] for r in requests] == [
        [f"{prefix}first"],
        [f"{prefix}second"],
    ]
    assert all("config" not in r for r in requests)


def test_gemini_v2_embed_query_text_still_truncates_before_the_prefix():
    """``truncate_queries`` ordering is unchanged by the rename: the body is
    cut first, then the prefix is prepended, so the combined payload stays
    under the model's hard cap."""
    requests = []
    ef = _gemini("models/gemini-embedding-2-preview", requests)

    ef.embed_query_text("a" * 50_000)

    sent = requests[0]["contents"][0]
    prefix = GeminiEmbeddingFunction.V2_QUERY_PREFIX
    assert sent.startswith(prefix)
    assert len(sent[len(prefix) :]) == 7980 * 4  # 4 chars/token estimate
