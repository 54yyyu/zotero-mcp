...
### Added
- **`zotero_set_item_parent` sets or clears an item's parent.** It can parent standalone notes and attachments, move them between parents, or make them top-level again. Contributed by @schmidma.
- **Parallel attachment extraction and a transient fulltext cache for `update-db` (#390).** `--extraction-workers N` (or `semantic_search.extraction.workers`) fans PDF text extraction out over a process pool — extraction is CPU-bound and the parser holds the GIL, so processes reach ~6x where threads managed ~1.1x. Workers receive only `(path, max_pages)` and return text; sqlite, the cache index and all ChromaDB writes stay in the parent. Extracted text is staged in a 0700/0600 on-disk cache keyed by attachment key, file mtime/size and extraction profile, evicted as soon as an item's chunks are embedded and persisted, so an interrupted run resumes without re-parsing what it already extracted. Default remains 1 worker (the exact historical sequential path) and the cache is written only by the indexing path, never by tool calls with different page caps. Contributed by @mronkko.
- **Submodules import without loading the MCP server.** `zotero_mcp.__init__` resolved `mcp` eagerly, so *any* `import zotero_mcp.<submodule>` paid ~1.7s and ~1480 modules for FastMCP, pydantic, pyzotero and friends before the submodule's own code ran. `mcp` now resolves lazily via PEP 562 (`from zotero_mcp import mcp` is unchanged, and still degrades to `AttributeError` when server deps are absent), and canonical DOI normalization moved to the new stdlib-only `zotero_mcp.identifiers.normalize_doi` (re-exported for existing callers). `from zotero_mcp.schema import valid_fields` now costs ~9ms and 75 modules. Contributed by @mronkko.

### Changed
- **The embedding functions moved into a `zotero_mcp.embeddings` provider package.** The OpenAI/Gemini/HuggingFace/Ollama embedding classes and the provider-resolution chains that lived inline in `chroma_client.py` now live in `embeddings/providers/` behind a registry; `chroma_client` re-exports the same class objects, so persisted collections, existing imports and monkeypatches keep working. Behavior-preserving refactor, no configuration changes. Contributed by @mronkko.

### Fixed
- **`if_exists` dedup now works over the Zotero Web API (#441).** The web API's quick search indexes only title, creator and year — never metadata fields — so `find_existing_items()` could not match a DOI/ISBN/arXiv ID over that transport and every re-add with `if_exists="skip"`/`"file"` silently created a duplicate. (The local API's `qmode=everything` does reach metadata, which is why this never surfaced there.) Once the metadata lookup has produced a title, a second quick search by title (`qmode=titleCreatorYear`, indexed on both transports) now runs, still confirmed by the same normalized client-side identifier comparison — a title collision alone can never count as a match. The bibtex/CSL batch path passes each entry's own title. Reported by @mronkko, with the transport-level diagnosis that made the fix obvious.
- **The transient fulltext cache stays transient on the OpenAI Batch API path.** `openai-batch-import` now evicts each item's cached text once its embeddings are persisted (as the realtime path already did), and `update-db` sweeps entries from abandoned runs, entries whose backing file is gone, and orphaned `.txt` files at the start of every invocation. Previously nothing on the batch path ever evicted, so the cache grew toward the extracted plaintext of the whole library until a manual `--clear-fulltext-cache`.
- **One dying extraction worker no longer marks every in-flight item as failed.** A worker killed mid-run (OOM on a pathological PDF, a segfaulting parser) breaks the whole process pool: every pending future raises `BrokenProcessPool`, and each casualty was recorded `has_fulltext="failed"` — which incremental sync then never retries — so one transient crash could poison thousands of items until `--force-rebuild`. Affected items are now re-run one at a time in fresh single-worker pools: isolation is preserved, innocents extract normally, and only the file that actually breaks its pool fails.
